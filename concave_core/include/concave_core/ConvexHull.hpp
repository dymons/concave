...
namespace concave {
	...
	std::vector<T> convex_hull(const std::vector<T>& t_points,
			typename std::enable_if_t<IsAlgorithm<AlgorithmHull<U>, AlgorithmHull<Pattern::JarvisMarch>>::value>*/*unused*/= 0)
	{
	...
	template <auto U, typename T>
	std::vector<T> convex_hull(const std::vector<T>& t_points,
			typename std::enable_if_t<IsAlgorithm<AlgorithmHull<U>, AlgorithmHull<Pattern::GrahamScan>>::value>*/*unused*/= 0)
	{
		std::vector<T> convex_hull;
	...
	std::vector<T> convex_hull(const std::vector<T>& t_points)
	{
		return convex_hull<Pattern::GrahamScan>(t_points);
	}
} // namespace concave
