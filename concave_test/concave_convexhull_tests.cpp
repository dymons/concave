...
TEST_F(ConvexHullTests, testJarvisMarch)
{
	...
	auto jarvis_march_empty { concave::convex_hull<concave::Pattern::JarvisMarch>(buffer) };
	EXPECT_TRUE(jarvis_march_empty.empty());

	/// Check little data
	buffer.insert(buffer.end(), m_points.begin(), std::next(m_points.begin(), 2));
	auto jarvis_march_bit { concave::convex_hull<concave::Pattern::JarvisMarch>(buffer) };
...
TEST_F(ConvexHullTests, algorithm_equivalence_test)
{
	...
	for (std::size_t i { 3 }; i<m_points.size()-1; ++i) {
		...
		auto jarvis_march { concave::convex_hull<concave::Pattern::JarvisMarch>(buffer) };
		auto quick_hull { concave::convex_hull<concave::Pattern::QuickHull>(buffer) };
		auto graham_scan { concave::convex_hull<concave::Pattern::GrahamScan>(buffer) };
